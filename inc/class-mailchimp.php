...
class MailchimpService
{
    ...
    public function subscribe(string $email, ?string $firstName, ?string $lastName, array $tags, string $status): array
    {
        ...
        if (in_array($currentStatus, $validStatuses, true)) {
            if (is_user_logged_in()) {
                ...
                return $this->handleResponse($response, __('Your subscription has been updated successfully.', 'gerendashaz'));
            }
            return $this->jsonResponse(true, __('Your e-mail address is already in our list.', 'gerendashaz'));
        }

        $response = $this->createMember($email, $firstName, $lastName, $tags);
        return $this->handleResponse($response, __('You have been subscribed successfully!', 'gerendashaz'));
    }
    ...
    {
        if (is_wp_error($response)) {
            return $this->jsonResponse(false, sprintf(__('Error: %s', 'gerendashaz'), $response->get_error_message()));
        }
