<?php
    ...
    if ( ! function_exists( 'dpg_register_menu' ) ) {
        ...
        function dpg_register_menu() {
            add_management_page(
                __('Dummy Post Generator', 'gerendashaz'), // Page title
                __('Dummy Post Generator', 'gerendashaz'), // Menu title
    ...
    if ( ! function_exists( 'dpg_admin_page' ) ) {
        ...
        function dpg_admin_page() {
            ?>
            <div class="wrap">
                <h1><?php echo esc_html__('Dummy Post Generator', 'gerendashaz'); ?></h1>

                <!-- Section: Generate Dummy Posts -->
                <form method="post">
                    <?php wp_nonce_field('dpg_generate_nonce', 'dpg_generate_nonce_field'); ?>
                    <h2><?php echo esc_html__('Generate Dummy Posts', 'gerendashaz'); ?></h2>
                    <table class="form-table">
                        <tr>
                            <th scope="row"><label for="dpg_post_type"><?php echo esc_html__('Select Post Type', 'gerendashaz'); ?></label></th>
                            <td>
                        ...
                        <tr>
                            <th scope="row"><label for="dpg_number"><?php echo esc_html__('Number of Posts', 'gerendashaz'); ?></label></th>
                            <td>
                                <input type="number" name="dpg_number" id="dpg_number" class="regular-text" value="5" min="1" max="100">
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="dpg_post_image_width"><?php echo esc_html__('Featured Image Width (px)', 'gerendashaz'); ?></label></th>
                            <td>
                                <input type="number" name="dpg_post_image_width" id="dpg_post_image_width" class="regular-text" value="1280" min="300" max="3840">
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="dpg_post_image_height"><?php echo esc_html__('Featured Image Height (px)', 'gerendashaz'); ?></label></th>
                            <td>
                                <input type="number" name="dpg_post_image_height" id="dpg_post_image_height" class="regular-text" value="720" min="300" max="2160">
                            </td>
                        </tr>
                    </table>
                    <?php submit_button(__('Generate Dummy Posts', 'gerendashaz')); ?>
                </form>

                <!-- Section: Upload Dummy Images -->
                <form method="post">
                    <?php wp_nonce_field('dpg_images_nonce', 'dpg_images_nonce_field'); ?>
                    <h2><?php echo esc_html__('Upload Dummy Images', 'gerendashaz'); ?></h2>
                    <table class="form-table">
                        <tr>
                            <th scope="row"><label for="dpg_image_count"><?php echo esc_html__('Number of Images', 'gerendashaz'); ?></label></th>
                            <td>
                                <input type="number" name="dpg_image_count" id="dpg_image_count" class="regular-text" value="5" min="1" max="100">
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="dpg_image_width"><?php echo esc_html__('Image Width (px)', 'gerendashaz'); ?></label></th>
                            <td>
                                <input type="number" name="dpg_image_width" id="dpg_image_width" class="regular-text" value="1280" min="300" max="3840">
                            </td>
                        </tr>
                        <tr>
                            <th scope="row"><label for="dpg_image_height"><?php echo esc_html__('Image Height (px)', 'gerendashaz'); ?></label></th>
                            <td>
                                <input type="number" name="dpg_image_height" id="dpg_image_height" class="regular-text" value="720" min="300" max="2160">
                            </td>
                        </tr>
                    </table>
                    <?php submit_button(__('Upload Dummy Images', 'gerendashaz')); ?>
                </form>
            </div>
            <?php
            ...
            if (isset($_POST['dpg_number'], $_POST['dpg_post_type'])) {
                if (!isset($_POST['dpg_generate_nonce_field']) || !wp_verify_nonce($_POST['dpg_generate_nonce_field'], 'dpg_generate_nonce')) {
                    wp_die(__('Security check failed.', 'gerendashaz'));
                }
