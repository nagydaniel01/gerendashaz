...
<form id="complaint_report_form" class="form form--contact" method="post" action="<?php echo esc_url( admin_url('admin-ajax.php') ); ?>" novalidate>
    <?php wp_nonce_field('complaint_report_form_action', 'complaint_report_form_nonce'); ?>
    ...
    <div class="mb-3">
        <label class="form-label">
            <?php echo esc_html__( 'Attachments', 'gerendashaz' ); ?>
        </label>
        <div id="<?php echo esc_attr($prefix); ?>dropzone" class="dropzone" data-input="<?php echo esc_attr($prefix); ?>files"></div>
        <input type="file" class="form-control" id="<?php echo esc_attr($prefix); ?>files" name="<?php echo esc_attr($prefix); ?>files[]" multiple hidden>
    </div>
